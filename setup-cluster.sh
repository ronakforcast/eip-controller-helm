...
# step_header <title> — prints header and asks to skip.
# Returns 1 to skip the step, 0 to proceed.
step_header() {
  header "$*"
  printf "${CYAN}  Skip this step? [y/N]: ${RESET}"
  read -r _skip
  case "$(echo "$_skip" | tr '[:upper:]' '[:lower:]')" in
    y|yes) info "Step skipped."; return 1 ;;
    *) return 0 ;;
  esac
}
...
# ─── Step 2: Node groups ──────────────────────────────────────────────────────
if step_header "Step 2 — Node groups"; then
...
if step_header "Step 3 — IMDSv2 hop limit = 2"; then
...
# ─── Step 4: EXTERNALSNAT ─────────────────────────────────────────────────────
if step_header "Step 4 — EXTERNALSNAT=true on aws-node DaemonSet"; then
...
if step_header "Step 5 — OIDC provider"; then
...
# ─── Step 6: IAM role ─────────────────────────────────────────────────────────
if step_header "Step 6 — IAM role"; then
...
# ─── Step 7: EIP quota ────────────────────────────────────────────────────────
if step_header "Step 7 — EIP quota"; then
...
# ─── Step 8: Helm install ─────────────────────────────────────────────────────
if step_header "Step 8 — Helm install"; then
  ...
  if [ "$HELM_STATUS" = "deployed" ]; then
    ...
    # get role ARN if not already set by step 6
    if [ -z "$ROLE_ARN" ]; then
      ROLE_ARN=$(aws iam get-role --role-name "$IAM_ROLE" \
        --query 'Role.Arn' --output text 2>/dev/null || echo "")
    fi
